...
foreach ($file in $files) {
    if (Test-Path $file) {
        ...
        $content = $content -replace "class TestingGame extends Game", "class TestingGame extends Game"
...
$html = Get-Content "testing_index.html" -Raw -Encoding UTF8
...
$tag = '<script type="module" src="js/testing_main.js"></script>'
$index = $html.IndexOf($tag)
if ($index -ge 0) {
    $html = $html.Substring(0, $index) + "<script>`n$jsContent`n</script>" + $html.Substring($index + $tag.Length)
}

Set-Content "testing_bundled.html" $html -Encoding UTF8
Write-Host "Bundled to testing_bundled.html"
